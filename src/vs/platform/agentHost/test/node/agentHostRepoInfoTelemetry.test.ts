...
suite('AgentHostRepoInfoTelemetry', () => {
	...
	test('withholds diff content when content exclusion may be active', async () => {
		const root = URI.file('/repo');
		let patchCalls = 0;
		const gitService: IAgentHostGitService = {
			...createNoopGitService(),
		...
		await collector.reportBegin({ ...restrictedContext, copilotIgnoreEnabled: true }, 'agent-session://copilot/s1', 'turn-1', root, undefined, () => true);

		assert.deepStrictEqual({
			patchCalls,
			report: reports[0] && {
				repoId: reports[0].repoId,
				fileRelativePaths: reports[0].fileRelativePaths,
				diffsJSON: reports[0].diffsJSON,
